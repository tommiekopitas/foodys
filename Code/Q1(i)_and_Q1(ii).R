...
first_time_flag <- FALSE # flag to print the first time we reach the acceptable 
# stdev
...
# deviation d
if (k > 100 & sqrt(S_sq/k) < d & !first_time_flag) {
  print(paste("We reach an acceptable s.d. of the estimator at ", k, 
              "iterations"))
  print(paste("Indeed, we see the Expected Time until Foodys is under-resourced 
              is:", round(mean(Total_T),digits = 2), "days (with k=",k,
              "iterations)"))
...
print(paste("The Expected Time until Foodys is under-resourced is:", 
            round(mean(Total_T),digits = 2), "days. This is with k=",
