...
# We initialise the vector to Inf (i.e. no breakdown time)
b_n <- rep(Inf, N)
b_n[1:n] <- t - (1/mu) * log(runif(n)) # We assign a breakdown time to the first
# 10 scooters that are working, the rest are not so have no breakdown time
...
while (flag){
  ...
  # and earliest check-up (this corresponds to t_{A,2} and t_{A,1} in our
  # report within our event list)
...
#========================
print(paste("The Expected Time until Foodys is under-resourced is:", 
            round(mean(Total_T),digits = 4), "days. This is with k=",
            length(Total_T),"iterations"))
print(paste("The current sample variance is ", round(sd(Total_T)**2, 
                                                     digits = 4)))
print(paste("The s.d. of the estimator is:", round(sd(Total_T)/sqrt(K), 
                                                   digits=4)))
print(paste("Max", round(max(Total_T),digits=2), "Min", round(min(Total_T),digits=2)))