...
######## Case 1: arrival-within T #################################
    ...
    A = c(A,t) # collect output data: A1(N_A) = t
...
# Let's calculate the average number of customers in the system.
# The way we will do this is to say that 0 customers were in the system x% 
# of the time, 1 customers y% of the time and so on. So first we need to 
# calculate the amount of time from the total T+T_p time that we are in each
# state. We do this below in the matrix time_at_state

time_at_state = matrix(0 , nrow = 0, ncol = 2) 

for (i in 1:(dim(ST)[1]-1)){
  
    time_at_state = rbind(time_at_state, c(ST[i,1], ST[i+1,4]-ST[i,4]))
  
}
# you can check that the sum(time_at_state[,2]) = T_p

# Now we can calculate the average no of customers in the system:
Av_customers_in_system  = (time_at_state[,1] %*% time_at_state[,2])/sum(time_at_state[,2])

print(paste("Average no of customers in the system = ", Av_customers_in_system))
